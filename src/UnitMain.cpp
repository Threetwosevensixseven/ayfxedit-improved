...
void __fastcall TFormMain::TitleUpdate(void)
{
	...
	//MAbout->Caption= "&About "+(AnsiString)TITLE_STR+"...";
}
...
void __fastcall TFormMain::MFileEffectLoadClick(TObject *Sender)
{
	...
	{
		const WCHAR* wc = OpenDialogEffect->FileName.c_str();
		_bstr_t b(wc);
		const char* c = b;
		if(EffectLoad(curEffect,c))
		{
			BankUpdateCount();
			DrawEdit(false);
			no_click=50;
		...
		{
			Application->MessageBox(L"Can't load effect!",L"Error",MB_OK);
		}
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TFormMain::MFileEffectSaveClick(TObject *Sender)
{
	...
	{
		Application->MessageBox(L"Effect is empty, nothing to save.",L"Save effect",MB_OK);
	}
	else
	{
		...
		{
			const WCHAR* wc = SaveDialogEffect->FileName.c_str();
			_bstr_t b(wc);
			const char* c = b;
			EffectSave(curEffect,c);
			no_click=50;
		}
	}
}
...
void __fastcall TFormMain::MImportWAVClick(TObject *Sender)
{
	...
	{
		const WCHAR* wc = OpenDialogImport->FileName.c_str();
		_bstr_t b(wc);
		const char* c = b;
		if(ImportWAV(curEffect,c))
		{
...
void __fastcall TFormMain::MImportPSGClick(TObject *Sender)
{
	...
	if(OpenDialogImport->Execute())
	{
		...
		if(import_ok)
		{
			...
			_bstr_t b(wc);
			const char* c = b;
			if(ImportPSG(curEffect,c,import_chan))
			{
				SaveDialogEffect->FileName="imported.afx";
				TitleUpdate();
				BankUpdateCount();
...
void __fastcall TFormMain::MImportVTXClick(TObject *Sender)
{
	...
	if(OpenDialogImport->Execute())
	{
		...
		if(import_ok)
		{
			...
			_bstr_t b(wc);
			const char* c = b;
			if(ImportVTX(curEffect,c,import_chan))
			{
				SaveDialogEffect->FileName="imported.afx";
				TitleUpdate();
				BankUpdateCount();
...
void __fastcall TFormMain::MFileMultiLoadClick(TObject *Sender)
{
	...
	if(OpenDialogMulti->Execute())
	{
		...
		for(aa=0;aa<OpenDialogMulti->Files->Count;aa++)
		{
			...
			const WCHAR* wc = OpenDialogMulti->Files->Strings[aa].c_str();
			_bstr_t b(wc);
			const char* c = b;
			if(!EffectLoad(pp,c))
			{
				Application->MessageBox((L"Can't load effect '"+OpenDialogMulti->Files->Strings[aa]+L"'!").c_str(),L"Error",MB_OK);
			}
			else
			{
...
void __fastcall TFormMain::MFileSaveBankClick(TObject *Sender)
{
	...
	{
		const WCHAR* wc = SaveDialogBank->FileName.c_str();
		_bstr_t b(wc);
		const char* c = b;
		BankSave(c,true);
		TitleUpdate();
...
void __fastcall TFormMain::MFileLoadBankClick(TObject *Sender)
{
	...
	{
		const WCHAR* wc = OpenDialogBank->FileName.c_str();
		_bstr_t b(wc);
		const char* c = b;
		if(BankLoad(c))
		{
		...
		{
			Application->MessageBox(L"Can't load bank!",L"Error",MB_OK);
		}
	}
}
//---------------------------------------------------------------------------
...
{
	const WCHAR* wc = EditEffectName->Text.c_str();
	_bstr_t b(wc);
	const char* c = b;
	strcpy(ayBank[curEffect].name,c);
}
//---------------------------------------------------------------------------
...
{
	if(Application->MessageBox(L"Delete effect?",L"Sure?",MB_YESNO)==IDYES)
	{
...
bool __fastcall TFormMain::MultiSaveExport(int type)
{
	...
	if(!cnt)
	{
		Application->MessageBox(L"Bank is empty, nothing to save or export.",L"Multi save/export",MB_OK);
	...
	for(id=0;id<allEffect;++id)
	{
		...
		if(FileExists(name))
		{
			if(Application->MessageBox((L"File '"+name+L"' already exists, overwrite?").c_str(),L"Confirm",MB_YESNO)==IDNO) continue;
		}
...
void __fastcall TFormMain::MImportVGMClick(TObject *Sender)
{
	...
	if(OpenDialogImport->Execute())
	{
		...
		if(import_ok)
		{
			...
			_bstr_t b(wc);
			const char* c = b;
			if(ImportVGM(curEffect,c,vgm_chan,vgm_noise))
			{
				SaveDialogEffect->FileName="imported.afx";
				TitleUpdate();
				BankUpdateCount();
...
void __fastcall TFormMain::MExportWavClick(TObject *Sender)
{
	if(!MExportAll->Checked)
	{
		...
		const WCHAR* wc = SaveDialogExport->FileName.c_str();
		_bstr_t b(wc);
		const char* c = b;
		EffectExportWAV(curEffect,c);
	}
	else
	{
...
void __fastcall TFormMain::MAboutClick(TObject *Sender)
{
	Application->MessageBox(UnicodeString(TITLE_STR).c_str(),L"About",MB_OK);
}
//---------------------------------------------------------------------------
