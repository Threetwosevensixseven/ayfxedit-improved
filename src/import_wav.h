...
bool ImportWAV(int effect,const char *filename)
{
	...
	{
		Application->MessageBox(L"This is not a WAVE file",L"Error",MB_OK);
		free(src_data);
		return false;
	}
	...
	{
		Application->MessageBox(L"Only unpacked PCM files supported",L"Error",MB_OK);
		free(src_data);
		return false;
	}
	...
	{
		Application->MessageBox(L"Only 8/16bit samples supported",L"Error",MB_OK);
		free(src_data);
		return false;
	}
	...
	if(ptr>=size)
	{
		Application->MessageBox(L"No DATA chunk in file",L"Error",MB_OK);
		free(src_data);
		return false;
	}
